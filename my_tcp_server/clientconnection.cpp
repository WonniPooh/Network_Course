...
void ClientConnection::slotNewMsg()
{
    ...
    QString* data_show = new QString;

    QDataStream in(client_socket);

    in.setVersion(QDataStream::Qt_5_5);

    QSharedPointer <QString> data_to_show(data_show);
    QSharedPointer <QString> data_to_forward(data_recieved);
    ...
    for(;;) // msg counter from exact client
    {
        ...
        if(data_recieved->isEmpty())
            return;

        *data_show = *data_recieved;

        emit displayMsg(client_id, data_to_show);
        ...
        for(int i = 0; i < vector_size; i++)
        {
            if(connections->contains(recipients[i]))
            {
                ...
                data.data_to_forward = data_to_forward;
...
{
    assert(income);
    int first_pos = income->indexOf("\"receiver_id\"");
    int last_pos = income->indexOf("]");

    if(first_pos != -1)
    {
        if(income[0][last_pos + 1] == QChar(','))
            last_pos++;
        if((*income)[first_pos - 1] == QChar(','))
            first_pos--;
        income->remove(first_pos,last_pos - first_pos + 1);
    }
}
...
QVector<int> ClientConnection::parseJson(QString* income_data)
{
  ...
  // Parse document
    ...
    QJsonArray recipients = json["receiver_id"].toArray();
