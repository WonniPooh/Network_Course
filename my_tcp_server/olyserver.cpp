...
OlyServer::OlyServer(QPlainTextEdit* server_status_field,
...
{
    m_edit->appendPlainText("Client \""+ m_names->getClientName(client_id) + "\" with ID" + QString::number(client_id)  + " connected.\n");
...
void OlyServer::slotClientDisconnected(quint16 client_id)
{
    ...
    map_access.unlock();
    m_edit->appendPlainText("Client with ID " + QString::number(client_id)  + " disconnected.\n");
}
